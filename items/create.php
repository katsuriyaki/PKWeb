...
<div class="form-card">
    <h1 class="form-card-title">Create New Item</h1>
    <form method="POST" action="">
        <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
        <div class="form-group">
            <label for="title" class="form-label">Title <span class="required">*</span></label>
            <input type="text" class="form-control" id="title" name="title" required maxlength="100" placeholder="Item title" value="<?= htmlEncode($_POST['title'] ?? '') ?>">
            <div class="form-hint">Max 100 characters</div>
        </div>
        <div class="form-group">
            <label for="description" class="form-label">Description</label>
            <textarea class="form-control" id="description" name="description" rows="5" placeholder="Optional description"><?= htmlEncode($_POST['description'] ?? '') ?></textarea>
        </div>
